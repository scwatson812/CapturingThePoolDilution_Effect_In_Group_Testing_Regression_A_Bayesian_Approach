...
###Sample the population regression spike and slab prior mixture probabilities
tau.samp <- function(){
  res = rbeta(Q,v.g + a.v,b.v - v.g + 1)
  return(res)
}

###Sample the population regression spike and slab prior mixture indicators
v.samp <-function(){
  den1 = dnorm(beta.g, rep(0,Q), sqrt(phi.g))		 
  den2 = dnorm(beta.g, rep(0,Q), sqrt(r.val*phi.g))
  
  #determine probability of success
  ratio = den1*tau.g/(den1*tau.g+den2*(1-tau.g))	
  vq = rbinom(Q,1,ratio)
  return(vq)
}

###Sample the prior slab variance
phi.samp <-function(){
  rv = v.g*rep(1,Q)+(1-v.g)*rep(r.val,Q)
  res = rinvgamma(Q, a.phi+1/2,beta.g^2/(2*rv)+b.phi) 
  return(res)
}

###Define spike and slab indicator function
r.func<- function(x){
  res = rep(r.val,length(x))
  res[x==1] = 1
  return(res)
}
...
#Helper function to calculate the probablity that a negative pool tests negative
...
#Helper function to calculate the probablity that a positive pool tests positive
...
####Calculate the probability that negative pool tests negative
...
###True individual disease statuts
Y.tilde.true = rbinom(N,1,p.true)
###True biomarker distribution parameters
theta.0.true = c(2.5,0.5)
theta.1.true = c(80,2)
###True biomarker concentration levels
...
range(zeta.tilde.true)

###Create Pools for Dorfman Testing  #####
###Pool size
c.size = 3
###Number of pools
J = N/c.size
...
range(W)
################################################################
#####  Specify Hyperparameters for Biomarker distributions #####
################################################################
N.cc = 100
...
###Hyperparameters for error variance
a.sigma.w = 3
b.sigma.w = 3
...
###Set up Tuning controls for Metropolis Hasting Steps
...
abline(v = ind.threshold.true)
pcoga(ind.threshold.true, rep(theta.0.true[1],1),rep(theta.0.true[2],1)*1)
pcoga(ind.threshold.true,c(rep(theta.0.true[1],1-1),theta.1.true[1]), c(rep(theta.0.true[2],1-1),theta.1.true[2])*1)
...
abline(v = pool.threshold.true)
pcoga(pool.threshold.true, rep(theta.0.true[1],c.size),rep(theta.0.true[2],c.size)*c.size)
pcoga(pool.threshold.true,c(rep(theta.0.true[1],c.size-1),theta.1.true[1]), c(rep(theta.0.true[2],c.size-1),theta.1.true[2])*c.size)
...
plot(zeta.tilde.hat - zeta.tilde.true)
...
ind.threshold.hat - ind.threshold.true
...
pool.threshold.hat - pool.threshold.true

#########################################
#####  Trace Plots for  First MCMC  #####
#########################################
plot(beta.array[1,])
...
for(j in 1:J){
  if(pool.status[j]){
    for(i in 1:length(mathcalP[[j]])){
      if(!is.element(mathcalP[[j]][i],retest.running.list)){
        mathcalP.retest[ct] = list(mathcalP[[j]][i])
        retest.running.list = c(retest.running.list,mathcalP[[j]][i])
        ct = ct + 1
      }
    }
  }
...
tau.array.original = tau.array
v.array.original = v.array
phi.array.original = phi.array
...
plot(zeta.tilde.hat.2 - zeta.tilde.true)
...
plot(W.tilde.hat.2 - W.tilde.true.2)
...
plot(Y.tilde.hat.2-Y.tilde.true)
...
ind.threshold.hat.2 - ind.threshold.true
...
pool.threshold.hat.2 - pool.threshold.true

#########################################
#####  Trace Plots for Second MCMC  #####
#########################################
plot(beta.array[1,])
abline(h = beta.true[1])
plot(beta.array[2,])
...
#####################################################################
#####  Identify Individuals to Retest Based on the Second MCMC  ####
#####################################################################
...
for(j in 1:J){
  if(pool.status[j]){
    for(i in 1:length(mathcalP[[j]])){
      if(!is.element(mathcalP[[j]][i],retest.running.list)){
        mathcalP.retest.2[ct] = list(mathcalP[[j]][i])
        ct = ct + 1
